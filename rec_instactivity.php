...
if(isset($_GET['responsibility']))
{
    ...
    $sql="DELETE FROM `instactivity` WHERE id='".$_SESSION['id']."';";
    $conn->query($sql);

    $stmt = $conn->prepare("INSERT INTO `instactivity` (`id`, `responsibility`, `organisation`, `start_date`, `end_date`) VALUES (?,?,?,?,?)");
    $stmt->bind_param("sssss", $_SESSION['id'], $responsibility , $responsibility , $start_date , $end_date);
    $stmt->execute();
}
$sql1 = "SELECT responsibility, responsibility, start_date, end_date FROM `instactivity` WHERE id='".$_SESSION['id']."';";
...
<html lang="en" class=" "><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    <title>Institute Activity</title>
