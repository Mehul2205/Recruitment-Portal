...
if(isset($_GET['degree']))
{
    ...
    $sql="DELETE FROM `masters` WHERE id='".$_SESSION['id']."';";
    $conn->query($sql);

    $stmt = $conn->prepare("INSERT INTO `masters` (`id`, `degree`, `discipline`, `institute`, `university`, `year_passed`, `date_of_result`, `marks`) VALUES (?,?,?,?,?,?,?,?)");
    $stmt->bind_param("ssssssss", $_SESSION['id'], $degree, $discipline, $institute, $university, $year_passed, $date_of_result, $marks);
    $stmt->execute();
}

$sql1 = "SELECT degree,discipline,institute,university,year_passed,date_of_result,marks FROM `masters` WHERE id='".$_SESSION['id']."';";
